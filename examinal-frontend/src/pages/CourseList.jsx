...
import { Plus, ArrowRight, BookOpen, Trash2 } from "lucide-react";
...
export default function CourseList() {
  ...
  const create = async (e) => {
    ...
    try { await API.post("/api/courses/", form); toast.success("Course created"); setShowCreate(false); setForm({ title: "", code: "", description: "" }); load(); }
  ...
  return (
    <div>
      <Header title="Courses" subtitle={user.role === "student" ? "Your enrolled courses" : "Manage your courses"}
        actions={canManage && <button className="btn-primary" onClick={() => setShowCreate(true)}><Plus size={18} /> New Course</button>} />

      {!courses.length ? (
        <div className="card p-20 text-center">
          <BookOpen size={44} className="mx-auto text-slate-300 mb-4" />
          <h3 className="text-lg font-semibold text-navy-800 text-navy-800 mb-1">No courses yet</h3>
          ...
          {courses.map((c) => (
            <div key={c.id} className="card p-6 hover:shadow-md transition-all duration-200 group">
              <div className="flex items-start justify-between mb-4">
                ...
                {(isInstructor || isAdmin) && <button onClick={() => del(c.id)} className="p-1.5 opacity-0 group-hover:opacity-100 hover:bg-red-50 rounded-lg transition-all"><Trash2 size={15} className="text-red-500" /></button>}
              </div>
