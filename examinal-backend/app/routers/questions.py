...
def check_marks_limit(exam_id: int, db: Session, additional_marks: float = 0, current_q_id: int = None):
    exam = db.query(Exam).filter(Exam.id == exam_id).first()
    if not exam:
        return
    
    query = db.query(ExamQuestion).filter(ExamQuestion.exam_id == exam_id)
    if current_q_id:
        query = query.filter(ExamQuestion.id != current_q_id)
    
    current_sum = sum(q.marks for q in query.all())
    if current_sum + additional_marks > exam.total_marks:
        raise HTTPException(
            status_code=400,
            detail=f"Marks limit exceeded. current: {current_sum}, adding: {additional_marks}, max: {exam.total_marks}"
        )
...
def create_question(payload: QuestionCreate, user: InstructorUser, db: Session = Depends(get_db)):  # type: ignore
    ...
    check_marks_limit(payload.exam_id, db, additional_marks=payload.marks)
...
def update_question(question_id: int, payload: QuestionUpdate, user: InstructorUser, db: Session = Depends(get_db)):  # type: ignore
    ...
    if payload.marks is not None:
        check_marks_limit(q.exam_id, db, additional_marks=payload.marks, current_q_id=question_id)
