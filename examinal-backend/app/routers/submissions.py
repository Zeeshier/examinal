...
def start_exam(
    ...
    # Check assignment OR approved enrollment request with valid key
    ...
    enrollment = (
        ...
        .filter(
            EnrollmentRequest.exam_id == exam.id,
            EnrollmentRequest.student_id == current_user.id,
            EnrollmentRequest.status == "approved",
            EnrollmentRequest.access_key == payload.access_key
        )
        .first()
    )

    if not assigned and not enrollment and current_user.role == "student":
        raise HTTPException(status_code=403, detail="Invalid access key or enrollment not approved")
